...
class BigbirdBlockSpareAttention(nn.Module):
    ...
    def forward(self, query_layer,
        ...
        rand_attn = torch.repeat_interleave(rand_attn, batch_size, 0).to(device)
        ...
        second_rand_pad = torch.cat(
            (torch.ones(b, h, wm, 4 * wn, device=device).long(), rand_mask[:, :, 0]), 3)
        ...
        second_last_rand_pad = torch.cat((
            torch.ones(b, h, wm, 4 * wn, device=device).long(), rand_mask[:, :, -1]), 3)
